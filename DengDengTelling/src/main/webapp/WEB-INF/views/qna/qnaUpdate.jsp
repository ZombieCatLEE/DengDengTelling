...
<div class="tab-box">
	<ul>
		...
		<li class="selected" onclick="location.href = '/qnaList'">Q&A</li>
		...
		<div style="height: auto; padding-bottom: 5em;">
			<div align="center" class="container">
				<div align="left" style="width: 100%; border: 1px solid #cccccc; padding: 2em 5em; margin-top: 20px; padding-bottom: 5em; margin-bottom: 1em; background-color: #ffffff">
					<span style="font-weight: bold; font-size: 1.3em;">Q&A 글수정</span>
					<hr />

					<div class="form-inline" style="margin-bottom: 1em;">
						<label style="margin-right: 30px;">글제목</label> 
						<label style="margin-right: 0px;"> 
							<select id="QNA_HEADER" name="QNA_HEADER" style="width: 8.5em">
								<option value="">말머리선택</option>
								<option value="펫시터문의"
									<c:if test="${map.QNA_HEADER eq '펫시터문의'}">selected</c:if>>펫시터문의</option>
								<option value="결제관련"
									<c:if test="${map.QNA_HEADER eq '결제관련'}">selected</c:if>>결제관련</option>
								<option value="이용취소"
									<c:if test="${map.QNA_HEADER eq '이용취소'}">selected</c:if>>이용취소</option>
								<option value="기타"
									<c:if test="${map.QNA_HEADER eq '기타'}">selected</c:if>>기타</option>
							</select>
						</label> 
						<input type="text" id="QNA_SUBJECT" name="QNA_SUBJECT" value="${map.QNA_SUBJECT }" class="form-control"
							style="width: 500px" placeholder="글 제목을 입력하세요." />
					</div>
					<div class="form-inline" style="margin-bottom: 1em">
						<label>글번호<span>${map.BOARD_NO}</span></label> 
						<label>작성자<span>${map.QNA_WRITER}</span></label>
						<label>조회수<span>${map.QNA_COUNT}</span></label> 
						<label>등록일<span>${map.QNA_DATE}</span></label>
					...
					<div>
						<textarea rows="20" cols="100" title="내용" id="QNA_CONTENT"
							style="resize: none;" name="QNA_CONTENT" placeholder="내용을 입력하세요.">${map.QNA_CONTENT}</textarea>
					</div>

					<div class="table file">
						<h1 style="font-size: 1.1em; font-weight: bold; margin-top: 5px">
							<img src="resources/images/fileUpload.png" style="width: 2.5em;">첨부 파일
						</h1>
				...
				<div style="position: relative; top: 1em;">
					...
					<div style="float: right; position: relative; right: 38em;">
						<a href="#this" class="btn_style" id="list"
							style="margin-right: 1em;">목록</a> <a href="#this"
							class="btn_style" id="update" style="margin-right: 1em;">수정</a> <a
							href="#this" class="btn_style" id="delete">삭제</a>
					</div>
				</div>
			</div>
		</div>
