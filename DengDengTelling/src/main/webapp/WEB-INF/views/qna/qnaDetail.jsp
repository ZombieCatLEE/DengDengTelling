	...
	<div style="height: auto; margin-bottom: 5em;">
		<div align="center" class="container">
			<div align="left"
				...
				<span style="font-weight: bold; font-size: 1.3em;">Q&A</span>
				<hr />
				<div class="form-inline" style="margin-bottom: 1em">
					<label style="margin-right: 100px;">말머리<span>${map.QNA_HEADER}</span></label>
					<label style="margin-right: 150px;">글제목<span>${map.QNA_SUBJECT}</span></label>
				</div>
				<div class="form-inline" style="margin-bottom: 1em">
					<label>글번호<span>${map.BOARD_NO}</span></label> <label>작성자<span>${map.QNA_WRITER}</span></label>
					<label>조회수<span>${map.QNA_COUNT}</span></label> <label>등록일<span>${map.QNA_DATE}</span></label>
					<input type="hidden" id="BOARD_TYPE" name="BOARD_TYPE"
						value="${map.BOARD_TYPE}"> <input type="hidden"
						id="BOARD_NO" name="BOARD_NO" value="${map.BOARD_NO}"> <input
						type="hidden" name="ID" id="ID" value="${map.ID}"> <input
						type="hidden" name="writerId" id="writerId" value="${ID}">
					<input type="hidden" name="QNA_RE_NO" id="QNA_RE_NO"
						value="${map.BOARD_NO}"> <input type="hidden"
						name="BOARD_CREA_ID" id="BOARD_CREA_ID" value="${map.ID}">
				</div>
				<div align="center">
					<div style="display: flex; border: 1px solid gray; height: auto; padding: 10px; display: block; width: 100%; margin-bottom: 10px;">
						...
						<div style="width: 80%; margin-top: 3em; margin-bottom: 3em; min-height: 230px; height: auto; font-size: 1.1em;" align="left">${map.QNA_CONTENT }</div>
					</div>
				</div>
...
<script type="text/javascript">
	...
	function fn_downloadFile(obj) {
		...
		comSubmit.setUrl("<c:url value='/qna/downloadFile' />");
