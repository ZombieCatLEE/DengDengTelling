...
// 매치 등록
	function matchModifySave() {
		var form = document.matchModify;
		...
		form.action = "/matchModifySave";
		form.submit();
	}	
</script>

<form id="matchModify" name="matchModify" method="post">
	...
	<c:if test="${msg != null}">
		<script type="text/javascript">
			alert("매칭 정보가 등록되어 있어 매칭 수정으로 이동합니다.");
		</script>
	</c:if>
	...
	<div class="container" style="text-align: center;">
		...
		<c:forEach var="pstIMG" items="${pstImg}" varStatus="status">
  			<div class="mySlides">
   	 			...
   	 			<img src="resources/downimage/<c:out value='${pstIMG.PSMEM_ORIGINAL_FILE_NAME}'/>" style="height: 600px; width: auto">
  			</div>
    	</c:forEach>
 		...
 		<div class="minirow" style="margin-top: 10px;" >
  			<c:forEach var="petIMAGE" items="${pstImg}" varStatus="status">
				<div class="minicolumn">
					<img class="demo cursor" src="resources/downimage/<c:out value='${petIMAGE.PSMEM_ORIGINAL_FILE_NAME}'/>" style="width: auto; height: 100px;" onclick="currentSlide(<c:out value='${status.count}'/>)" />
				</div>
	    	</c:forEach>
	    </div>
	</div>
		...
		<div style="float: left; width: 66%; padding: 40px;">
				...
				<div style="overflow: hidden; width: 400px;">
					...
					<p style="font-family: Noto Sans KR, sans-serif; margin-top: 5px; font-size: 25px; color: #383C48; line-height: 37px; letter-spacing: -0.2px;">
						<input style="font-family: Noto Sans KR, sans-serif; margin-top: 10px; font-size: 20px; color: #383C48; line-height: 1px;"
							type="text" id="SUBJECT" name="SUBJECT" value="${matchDtl.MATCH_SUBJECT}"/>
					</p>
			...
			<div style="margin-top: 58px">
				...
				<div style="display: flex; flex-direction: column; border-top: 1px solid rgb(235, 235, 235); padding: 10px 0px; ">
					<div style="display: flex; flex-direction: row; align-items: center;">
						<img width="200" height="100" src="resources/downimage/${certi.PROFILE_ORIGINAL_FILE_NAME}" style="object-fit: cover; margin-left: 20px; margin-right: 30px;">
			...
			<div style="margin-top: 58px">
				...
				<c:forEach var="petMem" items="${pet}" >
					<div style="display: flex; flex-direction: column; border-top: 1px solid rgb(235, 235, 235); padding: 10px 0px; ">
						<div style="display: flex; flex-direction: row; align-items: center;">
							<img width="60" height="60" src="resources/downimage/<c:out value='${petMem.STO_FILE_NAME}'/>" style="object-fit: cover; border-radius: 50%; margin-left: 20px; margin-right: 20px;">
							<div style="margin-left: 18px;">
								<!-- 글쓴이 -->
				...
				<div style="display: flex; flex-grow: 1; margin-top: 1em">
					<form method="get" class="tm-search-form tm-section-pad-2">
						<div class="form-group tm-form-element tm-form-element-50" style="width: 22em; right: 3em; bottom: 1em">
							...
							<input name="basicDate" type="text" class="flatpickr-input" id="basicDate" placeholder="펫시팅이 불가능한 날짜를 선택해주세요." style="font-size: 1em">
						</div>
					</form>
				</div>
				...
				<div style="display: flex; flex-direction: row; justify-content: space-between; margin-top: 38px">
					<div style="width: 149px">
						...
						<select id="checkInTime" name="checkInTime" style="width: 10.5em">
							...
							<option value="10:00"<c:if test="${matchDtl.MATCH_CHECK_IN eq '10:00'}">selected</c:if>>10:00</option>
							<option value="11:00"<c:if test="${matchDtl.MATCH_CHECK_IN eq '11:00'}">selected</c:if>>11:00</option>
							<option value="12:00"<c:if test="${matchDtl.MATCH_CHECK_IN eq '12:00'}">selected</c:if>>12:00</option>
							<option value="13:00"<c:if test="${matchDtl.MATCH_CHECK_IN eq '13:00'}">selected</c:if>>13:00</option>
							<option value="14:00"<c:if test="${matchDtl.MATCH_CHECK_IN eq '14:00'}">selected</c:if>>14:00</option>
							<option value="15:00"<c:if test="${matchDtl.MATCH_CHECK_IN eq '15:00'}">selected</c:if>>15:00</option>
							<option value="16:00"<c:if test="${matchDtl.MATCH_CHECK_IN eq '16:00'}">selected</c:if>>16:00</option>
							<option value="17:00"<c:if test="${matchDtl.MATCH_CHECK_IN eq '17:00'}">selected</c:if>>17:00</option>
							<option value="18:00"<c:if test="${matchDtl.MATCH_CHECK_IN eq '18:00'}">selected</c:if>>18:00</option>
							<option value="19:00"<c:if test="${matchDtl.MATCH_CHECK_IN eq '19:00'}">selected</c:if>>19:00</option>
							<option value="20:00"<c:if test="${matchDtl.MATCH_CHECK_IN eq '20:00'}">selected</c:if>>20:00</option>
							<option value="21:00"<c:if test="${matchDtl.MATCH_CHECK_IN eq '21:00'}">selected</c:if>>21:00</option>
							<option value="22:00"<c:if test="${matchDtl.MATCH_CHECK_IN eq '22:00'}">selected</c:if>>22:00</option>
						</select>
					</div>
					<div style="width: 149px">
						...
						<select id="checkOutTime" name="checkOutTime" style="width: 10.5em">
							<option value="선택">선택</option>
							<option value="10:00"<c:if test="${matchDtl.MATCH_CHECK_OUT eq '10:00'}">selected</c:if>>10:00</option>
							<option value="11:00"<c:if test="${matchDtl.MATCH_CHECK_OUT eq '11:00'}">selected</c:if>>11:00</option>
							<option value="12:00"<c:if test="${matchDtl.MATCH_CHECK_OUT eq '12:00'}">selected</c:if>>12:00</option>
							<option value="13:00"<c:if test="${matchDtl.MATCH_CHECK_OUT eq '13:00'}">selected</c:if>>13:00</option>
							<option value="14:00"<c:if test="${matchDtl.MATCH_CHECK_OUT eq '14:00'}">selected</c:if>>14:00</option>
							<option value="15:00"<c:if test="${matchDtl.MATCH_CHECK_OUT eq '15:00'}">selected</c:if>>15:00</option>
							<option value="16:00"<c:if test="${matchDtl.MATCH_CHECK_OUT eq '16:00'}">selected</c:if>>16:00</option>
							<option value="17:00"<c:if test="${matchDtl.MATCH_CHECK_OUT eq '17:00'}">selected</c:if>>17:00</option>
							<option value="18:00"<c:if test="${matchDtl.MATCH_CHECK_OUT eq '18:00'}">selected</c:if>>18:00</option>
							<option value="19:00"<c:if test="${matchDtl.MATCH_CHECK_OUT eq '19:00'}">selected</c:if>>19:00</option>
							<option value="20:00"<c:if test="${matchDtl.MATCH_CHECK_OUT eq '20:00'}">selected</c:if>>20:00</option>
							<option value="21:00"<c:if test="${matchDtl.MATCH_CHECK_OUT eq '21:00'}">selected</c:if>>21:00</option>
							<option value="22:00"<c:if test="${matchDtl.MATCH_CHECK_OUT eq '22:00'}">selected</c:if>>22:00</option>
						</select>
					</div>
				</div>
				<!-- 예약 시간 선택 끝 -->
						...
						<div style="display: flex; align-items: center; flex-direction: row">
							<p style="font-size: 12px; letter-spacing: 0.5px; line-height: 1px; color: #4E525B; margin-right: 11px">
								<select id="smallAll" name="smallAll" style="width: 7em; margin-right: 2px">
									<option value="선택">선택</option>
									<option value="30,000원"<c:if test="${matchDtl.PRICE_SMALL_ALL eq '30,000원'}">selected</c:if>>30,000원</option>
									<option value="29,000원"<c:if test="${matchDtl.PRICE_SMALL_ALL eq '29,000원'}">selected</c:if>>29,000원</option>
									<option value="28,000원"<c:if test="${matchDtl.PRICE_SMALL_ALL eq '28,000원'}">selected</c:if>>28,000원</option>
									<option value="27,000원"<c:if test="${matchDtl.PRICE_SMALL_ALL eq '27,000원'}">selected</c:if>>27,000원</option>
									<option value="26,000원"<c:if test="${matchDtl.PRICE_SMALL_ALL eq '26,000원'}">selected</c:if>>26,000원</option>
									<option value="25,000원"<c:if test="${matchDtl.PRICE_SMALL_ALL eq '25,000원'}">selected</c:if>>25,000원</option>
						...
						<div style="display: flex; flex-direction: column; align-items: center; width: 62px">
							<p style="font-size: 12px; letter-spacing: 0.5px; line-height: 20px; color: #4E525B">
								<select id="smallHalf" name="smallHalf" style="width: 7em">
									<option value="선택">선택</option>
									<option value="25,000원"<c:if test="${matchDtl.PRICE_SMALL_HALF eq '25,000원'}">selected</c:if>>25,000원</option>
									<option value="24,000원"<c:if test="${matchDtl.PRICE_SMALL_HALF eq '24,000원'}">selected</c:if>>24,000원</option>
									<option value="23,000원"<c:if test="${matchDtl.PRICE_SMALL_HALF eq '23,000원'}">selected</c:if>>23,000원</option>
									<option value="22,000원"<c:if test="${matchDtl.PRICE_SMALL_HALF eq '22,000원'}">selected</c:if>>22,000원</option>
									<option value="21,000원"<c:if test="${matchDtl.PRICE_SMALL_HALF eq '21,000원'}">selected</c:if>>21,000원</option>
									<option value="20,000원"<c:if test="${matchDtl.PRICE_SMALL_HALF eq '20,000원'}">selected</c:if>>20,000원</option>
									<!-- <option value="예약 불가">예약 불가</option> -->
								</select>
							</p>
						</div>
						...
						<div style="display: flex; align-items: center; flex-direction: row">
							<p style="font-size: 12px; letter-spacing: 0.5px; line-height: 1px; color: #4E525B; margin-right: 11px">
								<select id="mediumAll" name="mediumAll" style="width: 7em; margin-right: 2px">
									<option value="선택">선택</option>
									<option value="40,000원"<c:if test="${matchDtl.PRICE_MEDIUM_ALL eq '40,000원'}">selected</c:if>>40,000원</option>
									<option value="39,000원"<c:if test="${matchDtl.PRICE_MEDIUM_ALL eq '39,000원'}">selected</c:if>>39,000원</option>
									<option value="38,000원"<c:if test="${matchDtl.PRICE_MEDIUM_ALL eq '38,000원'}">selected</c:if>>38,000원</option>
									<option value="37,000원"<c:if test="${matchDtl.PRICE_MEDIUM_ALL eq '37,000원'}">selected</c:if>>37,000원</option>
									<option value="36,000원"<c:if test="${matchDtl.PRICE_MEDIUM_ALL eq '36,000원'}">selected</c:if>>36,000원</option>
									<option value="35,000원"<c:if test="${matchDtl.PRICE_MEDIUM_ALL eq '35,000원'}">selected</c:if>>35,000원</option>
									<!-- <option value="예약 불가">예약 불가</option> -->
								</select>
							</p>
						</div>
						<div style="display: flex; flex-direction: column; align-items: center; width: 62px">
							<p style="font-size: 12px; letter-spacing: 0.5px; line-height: 20px; color: #4E525B">
								<select id="mediumHalf" name="mediumHalf" style="width: 7em">
									<option value="선택">선택</option>
									<option value="35,000원"<c:if test="${matchDtl.PRICE_MEDIUM_HALF eq '35,000원'}">selected</c:if>>35,000원</option>
									<option value="34,000원"<c:if test="${matchDtl.PRICE_MEDIUM_HALF eq '34,000원'}">selected</c:if>>34,000원</option>
									<option value="33,000원"<c:if test="${matchDtl.PRICE_MEDIUM_HALF eq '33,000원'}">selected</c:if>>33,000원</option>
									<option value="32,000원"<c:if test="${matchDtl.PRICE_MEDIUM_HALF eq '32,000원'}">selected</c:if>>32,000원</option>
									<option value="31,000원"<c:if test="${matchDtl.PRICE_MEDIUM_HALF eq '31,000원'}">selected</c:if>>31,000원</option>
									<option value="30,000원"<c:if test="${matchDtl.PRICE_MEDIUM_HALF eq '30,000원'}">selected</c:if>>30,000원</option>
									<!-- <option value="예약 불가">예약 불가</option> -->
								</select>
							</p>
						</div>
						...
						<div style="display: flex; align-items: center; flex-direction: row">
							<p style="font-size: 12px; letter-spacing: 0.5px; line-height: 1px; color: #4E525B; margin-right: 11px">
								<select id="largeAll" name="largeAll" style="width: 7em; margin-right: 2px">
									<option value="선택">선택</option>
									<option value="50,000원"<c:if test="${matchDtl.PRICE_LARGE_ALL eq '50,000원'}">selected</c:if>>50,000원</option>
									<option value="49,000원"<c:if test="${matchDtl.PRICE_LARGE_ALL eq '49,000원'}">selected</c:if>>49,000원</option>
									<option value="48,000원"<c:if test="${matchDtl.PRICE_LARGE_ALL eq '48,000원'}">selected</c:if>>48,000원</option>
									<option value="47,000원"<c:if test="${matchDtl.PRICE_LARGE_ALL eq '47,000원'}">selected</c:if>>47,000원</option>
									<option value="46,000원"<c:if test="${matchDtl.PRICE_LARGE_ALL eq '46,000원'}">selected</c:if>>46,000원</option>
									<option value="45,000원"<c:if test="${matchDtl.PRICE_LARGE_ALL eq '45,000원'}">selected</c:if>>45,000원</option>
									<!-- <option value="예약 불가">예약 불가</option> -->
								</select>
							</p>
						</div>
						<div style="display: flex; flex-direction: column; align-items: center; width: 62px">
							<p style="font-size: 12px; letter-spacing: 0.5px; line-height: 20px; color: #4E525B">
								<select id="largeHalf" name="largeHalf" style="width: 7em">
									<option value="선택">선택</option>
									<option value="45,000원"<c:if test="${matchDtl.PRICE_LARGE_HALF eq '45,000원'}">selected</c:if>>45,000원</option>
									<option value="44,000원"<c:if test="${matchDtl.PRICE_LARGE_HALF eq '44,000원'}">selected</c:if>>44,000원</option>
									<option value="43,000원"<c:if test="${matchDtl.PRICE_LARGE_HALF eq '43,000원'}">selected</c:if>>43,000원</option>
									<option value="42,000원"<c:if test="${matchDtl.PRICE_LARGE_HALF eq '42,000원'}">selected</c:if>>42,000원</option>
									<option value="41,000원"<c:if test="${matchDtl.PRICE_LARGE_HALF eq '41,000원'}">selected</c:if>>41,000원</option>
									<option value="40,000원"<c:if test="${matchDtl.PRICE_LARGE_HALF eq '40,000원'}">selected</c:if>>40,000원</option>
									<!-- <option value="예약 불가">예약 불가</option> -->
								</select>
							</p>
						</div>
			...
			<div style="position: relative; line-height: 1em; display: flex; flex-direction: column; width: 375px; height: 400px; border-radius: 8px; border: 1px solid #DFE3EA; box-shadow: 1px 3px 7px rgba(0, 0, 0, 0.07); margin-top: 38px; overflow: hidden; text-align: center;">
				...
				<div id="map" style="position: absolute; height: 300px; border-radius: 4px; border: 1px solid #DFE3EA; overflow: hidden; display: inline-block; text-align: center; top: 23%; left: 52.5%; width: 95%; margin-top: -0.5em; margin-left: -50%;">
					...
					<script>
						...
						var address = matchModify.address1.value + ' ' + matchModify.address2.value
			...
			<div style="display: block; height: 80px; width: auto; position: relative; text-align: center; vertical-align: middle;">
				<div style="display: inline-block; height: auto; width: auto; text-align: center; position: relative; vertical-align: middle;">
					<div style="height: 30px; width: auto; margin-top: 10px; margin-left: 10px; margin-right: 10px; text-align: center; position: relative; vertical-align: middle;">
						<button type="button" style="height: 50px; width: 130px; font-size: 20px; font: bold; background-color: #037FBB; border: none; color:#fff; cursor: pointer; border-radius: 10px;" name="petsittingReserveBt" id="petsittingReserveBt" onclick="matchModifySave()">수정하기</button>
					</div>
				</div>
				<div style="display: inline-block; height: auto; width: auto; text-align: center; position: relative; vertical-align: middle;">
					<div style="height: 30px; width: auto; margin-top: 10px; margin-left: 10px; margin-right: 10px; text-align: center; position: relative; vertical-align: middle;">
						<button type="button" style="height: 50px; width: 130px; font-size: 20px; font: bold; background-color: #037FBB; border: none; color:#fff; cursor: pointer; border-radius: 10px;" onclick="history.back()">돌아가기</button>
					</div>
				</div>
			</div>
			</div>
